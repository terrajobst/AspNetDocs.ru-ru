...
title: Развертывание сайта с помощью FTP-клиента (VB) | Документация Майкрософт
...
ms.locfileid: "78438876"
---
# <a name="deploying-your-site-using-an-ftp-client-vb"></a>Развертывание сайта с помощью FTP-клиента (VB)

по [Скотт Митчелл](https://twitter.com/ScottOnWriting)

[Скачать код](https://download.microsoft.com/download/4/5/F/45F815EC-8B0E-46D3-9FB8-2DC015CCA306/ASPNET_Hosting_Tutorial_03_VB.zip) или [скачать PDF](https://download.microsoft.com/download/E/8/9/E8920AE6-D441-41A7-8A77-9EF8FF970D8B/aspnet_tutorial03_DeployingViaFTP_vb.pdf)
...
На рис. 1 показано FileZilla после копирования необходимых файлов. FileZilla отображает файлы на локальном компьютере слева, а также файлы на удаленном компьютере справа. Как показано на рис. 1, файлы исходного кода ASP.NET, например `About.aspx.vb`, находятся на локальном компьютере (среда разработки), но не были скопированы в поставщик веб-узла (Рабочая среда), так как файлы кода не нужно развертывать при явной компиляции.

> [!NOTE]
> Нет ничего негатива в наличии файлов исходного кода на рабочем сервере, так как они игнорируются. ASP.NET запрещает HTTP-запросы к файлам исходного кода по умолчанию, поэтому даже если файлы исходного кода находятся на рабочем сервере, они недоступны для посетителей веб-сайта. (Т. е. Если пользователь пытается посетить `http://www.yoursite.com/Default.aspx.vb` он получит страницу с сообщением об ошибке, в котором объясняется, что эти типы файлов `.vb` запрещены.)

[![использовать FTP-клиент для копирования необходимых файлов с рабочего стола на веб-сервер поставщика веб-узла.](deploying-your-site-using-an-ftp-client-vb/_static/image2.png)](deploying-your-site-using-an-ftp-client-vb/_static/image1.png)

**Рис. 1**. Использование FTP-клиента для копирования необходимых файлов с рабочего стола на веб-сервер поставщика веб-узла ([щелкните, чтобы просмотреть изображение с полным размером](deploying-your-site-using-an-ftp-client-vb/_static/image3.png))
...
[![сайт проверки книги теперь доступен всем, у кого есть подключение к Интернету.](deploying-your-site-using-an-ftp-client-vb/_static/image5.png)](deploying-your-site-using-an-ftp-client-vb/_static/image4.png)

**Рис. 2**. веб-сайт рецензирования книг теперь доступен любому пользователю с подключением к Интернету ([щелкните, чтобы просмотреть изображение с полным размером](deploying-your-site-using-an-ftp-client-vb/_static/image6.png))
...
На рис. 3 показано FileZilla после копирования необходимых файлов. Как видите, файлы с исходным кодом ASP.NET, такие как `About.aspx.vb`, существуют как на локальном компьютере (в среде разработки), так и в поставщике веб-узла (в рабочей среде), так как файлы кода необходимо развертывать при использовании автоматической компиляции.

[![использовать FTP-клиент для копирования необходимых файлов с рабочего стола на веб-сервер поставщика веб-узла](deploying-your-site-using-an-ftp-client-vb/_static/image8.png)](deploying-your-site-using-an-ftp-client-vb/_static/image7.png)

**Рис. 3**. Использование FTP-клиента для копирования необходимых файлов с рабочего стола на веб-сервер поставщика веб-узла ([щелкните, чтобы просмотреть изображение с полным размером](deploying-your-site-using-an-ftp-client-vb/_static/image9.png))
