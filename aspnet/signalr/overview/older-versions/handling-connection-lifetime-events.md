...
---
# <a name="understanding-and-handling-connection-lifetime-events-in-signalr-1x"></a>Основные сведения и обработка событий времени существования соединения в SignalR 1. x
...
> 
> В этой статье предполагается, что у вас уже есть знания о событиях SignalR и времени жизни подключения. Общие сведения о SignalR см. в разделе [SignalR-Overview-начало работы](index.md). Список событий времени жизни подключения см. в следующих ресурсах:
> 
> - [Как управлять событиями времени жизни соединения в классе HUB](index.md)
> - [Как управлять событиями времени жизни соединения в клиентах JavaScript](index.md)
> - [Как управлять событиями времени жизни соединения в клиентах .NET](index.md)
...
SignalR версии 1.1.1 не имеет встроенного API сервера для отключения клиентов. Существуют [планы по добавлению этой функции в будущем](https://github.com/SignalR/SignalR/issues/2101). В текущем выпуске SignalR самым простым способом отключения клиента от сервера является реализация метода отключения на клиенте и вызов этого метода с сервера. В следующем примере кода показан метод Disconnect для клиента JavaScript с использованием созданного прокси-сервера.
