...
`StockTickerHub` содержит ссылку на `StockTicker` Singleton, а `StockTicker` содержит ссылку на **ихубконнектионконтекст** для `StockTickerHub`. Он использует этот интерфейс для взаимодействия с экземплярами `StockTickerHub`. (Дополнительные сведения см. [в статье серверное вещание с помощью ASP.NET SignalR](../getting-started/tutorial-server-broadcast-with-signalr.md).)
...
Откройте класс Startup.cs приложения (созданный вручную в соответствии с инструкциями по пакету в `readme.txt`или созданный путем добавления проверки подлинности в проект). В методе `Startup.Configuration` создайте контейнер Нинжект, который Нинжект вызывает *ядро*.
...
Передайте сопоставитель зависимостей в метод **мапсигналр** , добавив конфигурацию концентратора:

[!code-csharp[Main](dependency-injection/samples/sample19.cs)]

Обновите метод Startup. Конфигуресигналр в классе Startup примера с помощью нового параметра:

[!code-csharp[Main](dependency-injection/samples/sample20.cs)]

Теперь SignalR будет использовать сопоставитель, указанный в **мапсигналр**, вместо распознавателя по умолчанию.

Ниже приведен полный листинг кода для `Startup.Configuration`.
...
Приложение имеет точно те же функциональные возможности, что и ранее. (Описание см. в разделе [серверное вещание с помощью ASP.NET SignalR](../getting-started/tutorial-server-broadcast-with-signalr.md).) Поведение не изменилось. просто сделать код проще в тестировании, обслуживании и развитии.
