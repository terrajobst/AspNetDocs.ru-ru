...
Так как вы добавили новое поле в класс `Movie`, вам также нужно обновить *список разрешений* привязки, чтобы включить это новое свойство. Обновите атрибут `bind` для методов действий `Create` и `Edit`, чтобы включить свойство `Rating`:
