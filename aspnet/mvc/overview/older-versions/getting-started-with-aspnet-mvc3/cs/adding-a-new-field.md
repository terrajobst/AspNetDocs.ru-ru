...
Теперь, когда вы обновили класс `Model`, вам также потребуется обновить шаблоны представления *\виевс\мовиес\индекс.кштмл* и *\виевс\мовиес\креате.кштмл* , чтобы обеспечить поддержку нового свойства `Rating`.
...
Назовите класс «Мовиеинитиализер». Обновите класс `MovieInitializer`, чтобы он содержал следующий код:
...
Давайте добавим два оператора using в начало файла. Первый элемент ссылается на пространство имен Entity Framework, а второй ссылается на пространство имен, где живет наш класс `MovieInitializer`:

[!code-csharp[Main](adding-a-new-field/samples/sample6.cs)]

Затем найдите метод `Application_Start` и добавьте вызов `Database.SetInitializer` в начале метода, как показано ниже:
