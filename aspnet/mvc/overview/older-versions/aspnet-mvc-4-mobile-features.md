...
Кроме того, вам потребуется эмулятор браузера для мобильных устройств. Любое из следующих действий будет работать:
...
Для этого раздела доступны C# проекты Visual Studio с исходным кодом:
...
Дисплей очень удобен для чтения на мобильном устройстве. Щелкните ссылку ASP.NET.
...
В следующем разделе вы узнаете, как предоставить представления, относящиеся к мобильным браузерам.

## <a name="overriding-views-layouts-and-partial-views"></a>Переопределение представлений, макетов и частичных представлений

Значительная новая функция ASP.NET MVC 4 — это простой механизм, позволяющий переопределять любое представление (включая макеты и частичные представления) для мобильных браузеров в целом, для отдельного браузера для мобильных устройств или для любого конкретного браузера. Чтобы предоставить представление для мобильных устройств, можно скопировать файл представления и добавить *. Mobile* на имя файла. Например, чтобы создать мобильное представление *индекса* , скопируйте *Views\Home\Index.cshtml* в *виевс\хоме\индекс.Мобиле.кштмл*.

В этом разделе вы создадите файл макета для мобильных устройств.
...
Перейдите на страницу Теги с помощью браузера для настольных систем и эмулятора браузера для мобильных устройств. В эмуляторе браузера для мобильных устройств показаны два внесенных изменения.
...
В дополнение к представлениям, относящимся к мобильным и настольным, можно создавать представления для отдельного браузера. Например, можно создавать представления, специально предназначенные для браузера iPhone. В этом разделе вы создадите макет для браузера iPhone и версию iPhone представления *AllTags* .

Откройте файл *Global. asax* и добавьте следующий код в метод `Application_Start`.

[!code-csharp[Main](aspnet-mvc-4-mobile-features/samples/sample7.cs)]

Этот код определяет новый режим экрана с именем iPhone, который будет сопоставляться с каждым входящим запросом. Если входящий запрос соответствует заданному условию (то есть если агент пользователя содержит строку "iPhone"), ASP.NET MVC будет искать представления, имя которых содержит суффикс "iPhone".

В коде щелкните правой кнопкой мыши `DefaultDisplayMode`, выберите команду **Разрешить**, а затем выберите `using System.Web.WebPages;`. При этом добавляется ссылка на пространство имен `System.Web.WebPages`, где определены типы `DisplayModes` и `DefaultDisplayMode`.

[![p2_resolve](aspnet-mvc-4-mobile-features/_static/image16.png)](aspnet-mvc-4-mobile-features/_static/image15.png)

Кроме того, можно просто добавить следующую строку в раздел `using` файла вручную.
...
Скопируйте файл *мвкмобиле\виевс\хоме\аллтагс.Мобиле.кштмл* в *мвкмобиле\виевс\хоме\аллтагс.ифоне.кштмл*. В новом файле измените элемент `<h2>` с "Tags (M)" на "Tags (iPhone)".

Запустите приложение. Запустите эмулятор браузера для мобильных устройств, убедитесь, что для его агента пользователя задано значение "iPhone", и перейдите к представлению *AllTags* . На следующем снимке экрана показано представление *AllTags* , отображаемое в браузере [Safari](http://www.apple.com/safari/download/) . Safari для Windows можно скачать [здесь](https://support.apple.com/kb/DL1531).
...
Создайте приложение и в эмуляторе браузера мобильного устройства перейдите к представлению *AllTags* . Вы увидите следующее:
...
В браузере мобильного устройства щелкните ссылку **динамики** . Так как отсутствует мобильное представление (*AllSpeakers. Mobile. cshtml*), динамики по умолчанию (*AllSpeakers. cshtml*) отображаются с использованием представления мобильного макета ( *\_Layout. Mobile. cshtml*).
...
Как вы только что видели, представление " *динамики* " доступно для чтения, но ссылки невелики и трудно коснуться на мобильном устройстве. В этом разделе вы создадите представление *колонок* для мобильных устройств, которое похоже на современное мобильное приложение — оно отображает большие и простые ссылки и содержит поле поиска для быстрого поиска докладчиков.
...
Код удаляет столбцы «комната» и «теги» и форматирует заголовок, докладчик и дату по вертикали, чтобы вся эта информация была доступна для чтения в браузере мобильного устройства. Изменения в коде отражены в приведенном ниже изображении.
...
Обновите браузер для мобильных устройств. На следующем рисунке отражены изменения кода, которые вы только что внесли:
...
- [Рекомендации консорциума W3C для мобильных веб-приложений](http://www.w3.org/TR/mwabp/)
- [Рекомендации консорциума W3C для запросов мультимедиа](http://www.w3.org/TR/css3-mediaqueries/)
