...
- Операции связаны с сетью или с границами ввода-вывода, а не с помощью ЦП.
- Параллелизм является более важным, чем простота кода.
- Необходимо предоставить механизм, позволяющий пользователям отменять долго выполняющийся запрос.
...
В следующем коде показаны методы `GetGizmos` и `GetGizmosAsync`.
