...
<span data-ttu-id="b169a-162">Часто приходится работать с объектом `Request`, который предоставляет сведения, такие как значения текстовых полей (полей формы) на странице, тип браузера, который сделал запрос, URL-адрес страницы, удостоверение пользователя и т. д. В следующем примере показано, как получить доступ к свойствам объекта `Request` и вызвать метод `MapPath` объекта `Request`, который предоставляет абсолютный путь к странице на сервере:</span><span class="sxs-lookup"><span data-stu-id="b169a-162">You'll often work with the `Request` object, which gives you information like the values of text boxes (form fields) on the page, what type of browser made the request, the URL of the page, the user identity, etc. The following example shows how to access properties of the `Request` object and how to call the `MapPath` method of the `Request` object, which gives you the absolute path of the page on the server:</span></span>
...
2. <span data-ttu-id="b169a-185">Скопируйте приведенный ниже код и разметку на страницу, заменив все, что уже есть на странице.</span><span class="sxs-lookup"><span data-stu-id="b169a-185">Copy the following code and markup into the page, replacing anything already in the page.</span></span>  
    ...
    <span data-ttu-id="b169a-186">Обратите внимание на некоторые моменты.</span><span class="sxs-lookup"><span data-stu-id="b169a-186">Here are some things for you to note:</span></span>
...
<span data-ttu-id="b169a-283">Для разметки, как вы, вероятно, знакомы, можно создать комментарий HTML:</span><span class="sxs-lookup"><span data-stu-id="b169a-283">For markup, as you probably know, you can create an HTML comment:</span></span>
...
<span data-ttu-id="b169a-370">Ниже приведен пример, который поможет понять различия:</span><span class="sxs-lookup"><span data-stu-id="b169a-370">Here's an example to help you understand the differences:</span></span>
...
<span data-ttu-id="b169a-405">Ключевое слово `if` запускает блок.</span><span class="sxs-lookup"><span data-stu-id="b169a-405">The `if` keyword starts a block.</span></span> <span data-ttu-id="b169a-406">Фактический тест (условие) задается в круглых скобках и возвращает значение true или false.</span><span class="sxs-lookup"><span data-stu-id="b169a-406">The actual test (condition) is in parentheses and returns true or false.</span></span> <span data-ttu-id="b169a-407">Инструкции, выполняемые, если тест имеет значение true, заключены в фигурные скобки.</span><span class="sxs-lookup"><span data-stu-id="b169a-407">The statements that run if the test is true are enclosed in braces.</span></span> <span data-ttu-id="b169a-408">Инструкция `if` может включать блок `else`, указывающий, какие операторы выполняются, если условие ложно:</span><span class="sxs-lookup"><span data-stu-id="b169a-408">An `if` statement can include an `else` block that specifies statements to run if the condition is false:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample50.cshtml)]

<span data-ttu-id="b169a-409">Можно добавить несколько условий с помощью блока `else if`.</span><span class="sxs-lookup"><span data-stu-id="b169a-409">You can add multiple conditions using an `else if` block:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample51.cshtml)]

<span data-ttu-id="b169a-410">В этом примере, если первое условие в блоке if не равно true, проверяется условие `else if`.</span><span class="sxs-lookup"><span data-stu-id="b169a-410">In this example, if the first condition in the if block is not true, the `else if` condition is checked.</span></span> <span data-ttu-id="b169a-411">Если это условие выполнено, выполняются инструкции в блоке `else if`.</span><span class="sxs-lookup"><span data-stu-id="b169a-411">If that condition is met, the statements in the `else if` block are executed.</span></span> <span data-ttu-id="b169a-412">Если ни одно из условий не соблюдается, выполняются инструкции в блоке `else`.</span><span class="sxs-lookup"><span data-stu-id="b169a-412">If none of the conditions are met, the statements in the `else` block are executed.</span></span> <span data-ttu-id="b169a-413">Можно добавить любое количество else if, а затем закрыть блок `else` как &quot;все остальное&quot; условие.</span><span class="sxs-lookup"><span data-stu-id="b169a-413">You can add any number of else if blocks, and then close with an `else` block as the &quot;everything else&quot; condition.</span></span>

<span data-ttu-id="b169a-414">Чтобы проверить большое количество условий, используйте блок `switch`:</span><span class="sxs-lookup"><span data-stu-id="b169a-414">To test a large number of conditions, use a `switch` block:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample52.cshtml)]

<span data-ttu-id="b169a-415">Проверяемое значение находится в круглых скобках (в примере это переменная `weekday`).</span><span class="sxs-lookup"><span data-stu-id="b169a-415">The value to test is in parentheses (in the example, the `weekday` variable).</span></span> <span data-ttu-id="b169a-416">Каждый отдельный тест использует инструкцию `case`, которая заканчивается двоеточием (:).</span><span class="sxs-lookup"><span data-stu-id="b169a-416">Each individual test uses a `case` statement that ends with a colon (:).</span></span> <span data-ttu-id="b169a-417">Если значение инструкции `case` соответствует тестовому значению, выполняется код в этом блоке Case.</span><span class="sxs-lookup"><span data-stu-id="b169a-417">If the value of a `case` statement matches the test value, the code in that case block is executed.</span></span> <span data-ttu-id="b169a-418">Каждый оператор Case закрывается с помощью оператора `break`.</span><span class="sxs-lookup"><span data-stu-id="b169a-418">You close each case statement with a `break` statement.</span></span> <span data-ttu-id="b169a-419">(Если вы забыли включить break в каждом блоке `case`, будет также выполняться код из следующей `case`.) Блок `switch` часто содержит оператор `default`, как в последнем случае &quot;все остальное&quot;, которое выполняется, если ни одно из других случаев не выполняется.</span><span class="sxs-lookup"><span data-stu-id="b169a-419">(If you forget to include break in each `case` block, the code from the next `case` statement will run also.) A `switch` block often has a `default` statement as the last case for an &quot;everything else&quot; option that runs if none of the other cases are true.</span></span>

<span data-ttu-id="b169a-420">Результат двух последних условных блоков, отображаемых в браузере:</span><span class="sxs-lookup"><span data-stu-id="b169a-420">The result of the last two conditional blocks displayed in a browser:</span></span>

![Razor — Img10](introducing-razor-syntax-c/_static/image10.jpg)

### <a name="looping-code"></a><span data-ttu-id="b169a-422">Циклическое выполнение кода</span><span class="sxs-lookup"><span data-stu-id="b169a-422">Looping Code</span></span>

<span data-ttu-id="b169a-423">Часто приходится многократно выполнять одни и те же инструкции.</span><span class="sxs-lookup"><span data-stu-id="b169a-423">You often need to run the same statements repeatedly.</span></span> <span data-ttu-id="b169a-424">Для этого нужно выполнить цикл.</span><span class="sxs-lookup"><span data-stu-id="b169a-424">You do this by looping.</span></span> <span data-ttu-id="b169a-425">Например, часто выполняются одни и те же инструкции для каждого элемента в коллекции данных.</span><span class="sxs-lookup"><span data-stu-id="b169a-425">For example, you often run the same statements for each item in a collection of data.</span></span> <span data-ttu-id="b169a-426">Если точно известно, сколько раз вы хотите пройти цикл, можно использовать цикл `for`.</span><span class="sxs-lookup"><span data-stu-id="b169a-426">If you know exactly how many times you want to loop, you can use a `for` loop.</span></span> <span data-ttu-id="b169a-427">Этот тип цикла особенно полезен для подсчета или пересчета:</span><span class="sxs-lookup"><span data-stu-id="b169a-427">This kind of loop is especially useful for counting up or counting down:</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample53.html)]

<span data-ttu-id="b169a-428">Цикл начинается с ключевого слова `for`, за которым следуют три оператора в круглых скобках, каждый из которых завершается точкой с запятой.</span><span class="sxs-lookup"><span data-stu-id="b169a-428">The loop begins with the `for` keyword, followed by three statements in parentheses, each terminated with a semicolon.</span></span>

- <span data-ttu-id="b169a-429">В круглых скобках первый оператор (`var i=10;`) создает счетчик и инициализирует его до 10.</span><span class="sxs-lookup"><span data-stu-id="b169a-429">Inside the parentheses, the first statement (`var i=10;`) creates a counter and initializes it to 10.</span></span> <span data-ttu-id="b169a-430">Вам не нужно присвоить значение счетчику, `i` &#8212; можно использовать любую переменную.</span><span class="sxs-lookup"><span data-stu-id="b169a-430">You don't have to name the counter `i` &#8212; you can use any variable.</span></span> <span data-ttu-id="b169a-431">При выполнении цикла `for` автоматически увеличивается значение счетчика.</span><span class="sxs-lookup"><span data-stu-id="b169a-431">When the `for` loop runs, the counter is automatically incremented.</span></span>
- <span data-ttu-id="b169a-432">Вторая инструкция (`i < 21;`) задает условие того, насколько вы хотите подсчитать.</span><span class="sxs-lookup"><span data-stu-id="b169a-432">The second statement (`i < 21;`) sets the condition for how far you want to count.</span></span> <span data-ttu-id="b169a-433">В этом случае вы хотите, чтобы он не превышал значение 20 (то есть, пока значение счетчика меньше 21).</span><span class="sxs-lookup"><span data-stu-id="b169a-433">In this case, you want it to go to a maximum of 20 (that is, keep going while the counter is less than 21).</span></span>
- <span data-ttu-id="b169a-434">Третья инструкция (`i++`) использует оператор инкремента, который просто указывает, что счетчик должен иметь 1 к нему каждый раз при выполнении цикла.</span><span class="sxs-lookup"><span data-stu-id="b169a-434">The third statement (`i++` ) uses an increment operator, which simply specifies that the counter should have 1 added to it each time the loop runs.</span></span>

<span data-ttu-id="b169a-435">Внутри фигурных скобок находится код, который будет выполняться для каждой итерации цикла.</span><span class="sxs-lookup"><span data-stu-id="b169a-435">Inside the braces is the code that will run for each iteration of the loop.</span></span> <span data-ttu-id="b169a-436">Разметка создает новый абзац (`<p>` элемент) каждый раз и добавляет строку к выходу, отображая значение `i` (счетчик).</span><span class="sxs-lookup"><span data-stu-id="b169a-436">The markup creates a new paragraph (`<p>` element) each time and adds a line to the output, displaying the value of `i` (the counter).</span></span> <span data-ttu-id="b169a-437">При запуске этой страницы в примере создаются 11 строк, в которых отображаются выходные данные, и текст в каждой строке, указывающей на номер товара.</span><span class="sxs-lookup"><span data-stu-id="b169a-437">When you run this page, the example creates 11 lines displaying the output, with the text in each line indicating the item number.</span></span>

![Razor — Img11](introducing-razor-syntax-c/_static/image11.jpg)

<span data-ttu-id="b169a-439">При работе с коллекцией или массивом часто используется цикл `foreach`.</span><span class="sxs-lookup"><span data-stu-id="b169a-439">If you're working with a collection or array, you often use a `foreach` loop.</span></span> <span data-ttu-id="b169a-440">Коллекция — это группа схожих объектов, а цикл `foreach` позволяет выполнять задачу для каждого элемента в коллекции.</span><span class="sxs-lookup"><span data-stu-id="b169a-440">A collection is a group of similar objects, and the `foreach` loop lets you carry out a task on each item in the collection.</span></span> <span data-ttu-id="b169a-441">Этот тип цикла удобен для коллекций, так как в отличие от цикла `for` не нужно увеличивать счетчик или устанавливать ограничение.</span><span class="sxs-lookup"><span data-stu-id="b169a-441">This type of loop is convenient for collections, because unlike a `for` loop, you don't have to increment the counter or set a limit.</span></span> <span data-ttu-id="b169a-442">Вместо этого код цикла `foreach` просто проходит по коллекции, пока не завершится.</span><span class="sxs-lookup"><span data-stu-id="b169a-442">Instead, the `foreach` loop code simply proceeds through the collection until it's finished.</span></span>

<span data-ttu-id="b169a-443">Например, следующий код возвращает элементы коллекции `Request.ServerVariables`, которая представляет собой объект, содержащий сведения о веб-сервере.</span><span class="sxs-lookup"><span data-stu-id="b169a-443">For example, the following code returns the items in the `Request.ServerVariables` collection, which is an object that contains information about your web server.</span></span> <span data-ttu-id="b169a-444">В нем используется цикл `foreac` h для вывода имени каждого элемента путем создания нового элемента `<li>` в маркированном списке HTML.</span><span class="sxs-lookup"><span data-stu-id="b169a-444">It uses a `foreac` h loop to display the name of each item by creating a new `<li>` element in an HTML bulleted list.</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample54.html)]

<span data-ttu-id="b169a-445">За ключевым словом `foreach` следуют круглые скобки, в которых объявляется переменная, представляющая один элемент в коллекции (в примере — `var item`), за которой следует ключевое слово `in`, а затем Коллекция, которую необходимо прокручивать.</span><span class="sxs-lookup"><span data-stu-id="b169a-445">The `foreach` keyword is followed by parentheses where you declare a variable that represents a single item in the collection (in the example, `var item`), followed by the `in` keyword, followed by the collection you want to loop through.</span></span> <span data-ttu-id="b169a-446">В теле цикла `foreach` можно получить доступ к текущему элементу с помощью объявленной ранее переменной.</span><span class="sxs-lookup"><span data-stu-id="b169a-446">In the body of the `foreach` loop, you can access the current item using the variable that you declared earlier.</span></span>

![Razor — Img12](introducing-razor-syntax-c/_static/image12.jpg)

<span data-ttu-id="b169a-448">Чтобы создать более общий цикл общего назначения, используйте оператор `while`:</span><span class="sxs-lookup"><span data-stu-id="b169a-448">To create a more general-purpose loop, use the `while` statement:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample55.cshtml)]

<span data-ttu-id="b169a-449">Цикл `while` начинается с ключевого слова `while`, за которым следуют круглые скобки, в которых указывается, как долго будет продолжаться цикл (в случае, если `countNum` меньше 50), а затем для повторения блока.</span><span class="sxs-lookup"><span data-stu-id="b169a-449">A `while` loop begins with the `while` keyword, followed by parentheses where you specify how long the loop continues (here, for as long as `countNum` is less than 50), then the block to repeat.</span></span> <span data-ttu-id="b169a-450">Циклы обычно увеличивают (добавляются в) или уменьшают (вычитаются из) переменную или объект, используемый для подсчета.</span><span class="sxs-lookup"><span data-stu-id="b169a-450">Loops typically increment (add to) or decrement (subtract from) a variable or object used for counting.</span></span> <span data-ttu-id="b169a-451">В этом примере оператор `+=` добавляет 1 к `countNum` при каждом запуске цикла.</span><span class="sxs-lookup"><span data-stu-id="b169a-451">In the example, the `+=` operator adds 1 to `countNum` each time the loop runs.</span></span> <span data-ttu-id="b169a-452">(Чтобы уменьшить переменную в цикле, который подсчитывается вниз, используйте оператор декремента `-=`).</span><span class="sxs-lookup"><span data-stu-id="b169a-452">(To decrement a variable in a loop that counts down, you would use the decrement operator `-=`).</span></span>

## <a name="objects-and-collections"></a><span data-ttu-id="b169a-453">Объекты и коллекции</span><span class="sxs-lookup"><span data-stu-id="b169a-453">Objects and Collections</span></span>

<span data-ttu-id="b169a-454">Практически все на веб-сайте ASP.NET — это объект, включая саму веб-страницу.</span><span class="sxs-lookup"><span data-stu-id="b169a-454">Nearly everything in an ASP.NET website is an object, including the web page itself.</span></span> <span data-ttu-id="b169a-455">В этом разделе обсуждаются некоторые важные объекты, с которыми часто приходится работать в коде.</span><span class="sxs-lookup"><span data-stu-id="b169a-455">This section discusses some important objects you'll work with frequently in your code.</span></span>

### <a name="page-objects"></a><span data-ttu-id="b169a-456">Объекты Page</span><span class="sxs-lookup"><span data-stu-id="b169a-456">Page Objects</span></span>

<span data-ttu-id="b169a-457">Самым основным объектом в ASP.NET является страница.</span><span class="sxs-lookup"><span data-stu-id="b169a-457">The most basic object in ASP.NET is the page.</span></span> <span data-ttu-id="b169a-458">Вы можете обращаться к свойствам объекта Page напрямую, не используя соответствующий объект.</span><span class="sxs-lookup"><span data-stu-id="b169a-458">You can access properties of the page object directly without any qualifying object.</span></span> <span data-ttu-id="b169a-459">Следующий код получает путь к файлу страницы, используя объект `Request` страницы:</span><span class="sxs-lookup"><span data-stu-id="b169a-459">The following code gets the page's file path, using the `Request` object of the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample56.cshtml)]

<span data-ttu-id="b169a-460">Чтобы ясно, что вы ссылаетесь на свойства и методы объекта текущей страницы, при необходимости можно использовать ключевое слово `this` для представления объекта страницы в коде.</span><span class="sxs-lookup"><span data-stu-id="b169a-460">To make it clear that you're referencing properties and methods on the current page object, you can optionally use the keyword `this` to represent the page object in your code.</span></span> <span data-ttu-id="b169a-461">Ниже приведен предыдущий пример кода с `this` добавленным для представления страницы:</span><span class="sxs-lookup"><span data-stu-id="b169a-461">Here is the previous code example, with `this` added to represent the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample57.cshtml)]

<span data-ttu-id="b169a-462">Вы можете использовать свойства объекта `Page` для получения большого объема информации, например:</span><span class="sxs-lookup"><span data-stu-id="b169a-462">You can use properties of the `Page` object to get a lot of information, such as:</span></span>

- <span data-ttu-id="b169a-463">`Request`.</span><span class="sxs-lookup"><span data-stu-id="b169a-463">`Request`.</span></span> <span data-ttu-id="b169a-464">Как уже было показано, это коллекция сведений о текущем запросе, включая тип браузера, который сделал запрос, URL-адрес страницы, удостоверение пользователя и т. д.</span><span class="sxs-lookup"><span data-stu-id="b169a-464">As you've already seen, this is a collection of information about the current request, including what type of browser made the request, the URL of the page, the user identity, etc.</span></span>
- <span data-ttu-id="b169a-465">`Response`.</span><span class="sxs-lookup"><span data-stu-id="b169a-465">`Response`.</span></span> <span data-ttu-id="b169a-466">Это коллекция сведений о ответе (странице), которая будет отправлена в браузер после завершения выполнения кода сервера.</span><span class="sxs-lookup"><span data-stu-id="b169a-466">This is a collection of information about the response (page) that will be sent to the browser when the server code has finished running.</span></span> <span data-ttu-id="b169a-467">Например, это свойство можно использовать для записи сведений в ответ.</span><span class="sxs-lookup"><span data-stu-id="b169a-467">For example, you can use this property to write information into the response.</span></span> 

    [!code-cshtml[Main](introducing-razor-syntax-c/samples/sample58.cshtml)]

<a id="ID_CollectionsAndObjects"></a>
### <a name="collection-objects-arrays-and-dictionaries"></a><span data-ttu-id="b169a-468">Объекты коллекции (массивы и словари)</span><span class="sxs-lookup"><span data-stu-id="b169a-468">Collection Objects (Arrays and Dictionaries)</span></span>

<span data-ttu-id="b169a-469">*Коллекция* — это группа объектов одного типа, например коллекция объектов `Customer` из базы данных.</span><span class="sxs-lookup"><span data-stu-id="b169a-469">A *collection* is a group of objects of the same type, such as a collection of `Customer` objects from a database.</span></span> <span data-ttu-id="b169a-470">ASP.NET содержит множество встроенных коллекций, таких как коллекция `Request.Files`.</span><span class="sxs-lookup"><span data-stu-id="b169a-470">ASP.NET contains many built-in collections, like the `Request.Files` collection.</span></span>

<span data-ttu-id="b169a-471">Вы часто работаете с данными в коллекциях.</span><span class="sxs-lookup"><span data-stu-id="b169a-471">You'll often work with data in collections.</span></span> <span data-ttu-id="b169a-472">Два общих типа коллекций — это *массив* и *словарь*.</span><span class="sxs-lookup"><span data-stu-id="b169a-472">Two common collection types are the *array* and the *dictionary*.</span></span> <span data-ttu-id="b169a-473">Массив полезен, если нужно сохранить коллекцию схожих элементов, но не нужно создавать отдельную переменную для хранения каждого элемента:</span><span class="sxs-lookup"><span data-stu-id="b169a-473">An array is useful when you want to store a collection of similar items but don't want to create a separate variable to hold each item:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample59.cshtml)]

<span data-ttu-id="b169a-474">При использовании массивов вы объявляете конкретный тип данных, например `string`, `int`или `DateTime`.</span><span class="sxs-lookup"><span data-stu-id="b169a-474">With arrays, you declare a specific data type, such as `string`, `int`, or `DateTime`.</span></span> <span data-ttu-id="b169a-475">Чтобы указать, что переменная может содержать массив, добавьте в объявление скобки (например, `string[]` или `int[]`).</span><span class="sxs-lookup"><span data-stu-id="b169a-475">To indicate that the variable can contain an array, you add brackets to the declaration (such as `string[]` or `int[]`).</span></span> <span data-ttu-id="b169a-476">Доступ к элементам массива можно получить с помощью их позиции (индекса) или с помощью инструкции `foreach`.</span><span class="sxs-lookup"><span data-stu-id="b169a-476">You can access items in an array using their position (index) or by using the `foreach` statement.</span></span> <span data-ttu-id="b169a-477">Индексы массива отсчитываются от &#8212; нуля, т. е. первый элемент находится в позиции 0, второй элемент — в позиции 1 и т. д.</span><span class="sxs-lookup"><span data-stu-id="b169a-477">Array indexes are zero-based &#8212; that is, the first item is at position 0, the second item is at position 1, and so on.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample60.cshtml)]

<span data-ttu-id="b169a-478">Чтобы определить количество элементов в массиве, можно получить свойство `Length`.</span><span class="sxs-lookup"><span data-stu-id="b169a-478">You can determine the number of items in an array by getting its `Length` property.</span></span> <span data-ttu-id="b169a-479">Чтобы получить позицию определенного элемента в массиве (для поиска в массиве), используйте метод `Array.IndexOf`.</span><span class="sxs-lookup"><span data-stu-id="b169a-479">To get the position of a specific item in the array (to search the array), use the `Array.IndexOf` method.</span></span> <span data-ttu-id="b169a-480">Можно также выполнять такие действия, как обратный порядок содержимого массива (метод `Array.Reverse`) или сортировать содержимое (метод `Array.Sort`).</span><span class="sxs-lookup"><span data-stu-id="b169a-480">You can also do things like reverse the contents of an array (the `Array.Reverse` method) or sort the contents (the `Array.Sort` method).</span></span>

<span data-ttu-id="b169a-481">Выходные данные кода массива строк, отображаемого в браузере:</span><span class="sxs-lookup"><span data-stu-id="b169a-481">The output of the string array code displayed in a browser:</span></span>

![Razor — Img13](introducing-razor-syntax-c/_static/image13.jpg)

<span data-ttu-id="b169a-483">Словарь — это коллекция пар "ключ-значение", где вы предоставляете ключ (или имя) для установки или получения соответствующего значения:</span><span class="sxs-lookup"><span data-stu-id="b169a-483">A dictionary is a collection of key/value pairs, where you provide the key (or name) to set or retrieve the corresponding value:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample61.cshtml)]

<span data-ttu-id="b169a-484">Чтобы создать словарь, используйте ключевое слово `new`, чтобы указать, что вы создаете новый объект Dictionary.</span><span class="sxs-lookup"><span data-stu-id="b169a-484">To create a dictionary, you use the `new` keyword to indicate that you're creating a new dictionary object.</span></span> <span data-ttu-id="b169a-485">Можно назначить словарь переменной с помощью ключевого слова `var`.</span><span class="sxs-lookup"><span data-stu-id="b169a-485">You can assign a dictionary to a variable using the `var` keyword.</span></span> <span data-ttu-id="b169a-486">Типы данных элементов в словаре указываются с помощью угловых скобок (`< >`).</span><span class="sxs-lookup"><span data-stu-id="b169a-486">You indicate the data types of the items in the dictionary using angle brackets ( `< >` ).</span></span> <span data-ttu-id="b169a-487">В конце объявления необходимо добавить пару круглых скобок, поскольку это фактически метод, создающий новый словарь.</span><span class="sxs-lookup"><span data-stu-id="b169a-487">At the end of the declaration, you must add a pair of parentheses, because this is actually a method that creates a new dictionary.</span></span>

<span data-ttu-id="b169a-488">Чтобы добавить элементы в словарь, можно вызвать метод `Add` переменной Dictionary (`myScores` в этом случае), а затем указать ключ и значение.</span><span class="sxs-lookup"><span data-stu-id="b169a-488">To add items to the dictionary, you can call the `Add` method of the dictionary variable (`myScores` in this case), and then specify a key and a value.</span></span> <span data-ttu-id="b169a-489">Кроме того, можно использовать квадратные скобки для обозначения ключа и выполнить простое назначение, как показано в следующем примере:</span><span class="sxs-lookup"><span data-stu-id="b169a-489">Alternatively, you can use square brackets to indicate the key and do a simple assignment, as in the following example:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample62.cs)]

<span data-ttu-id="b169a-490">Чтобы получить значение из словаря, необходимо указать ключ в квадратных скобках:</span><span class="sxs-lookup"><span data-stu-id="b169a-490">To get a value from the dictionary, you specify the key in brackets:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample63.cs)]

## <a name="calling-methods-with-parameters"></a><span data-ttu-id="b169a-491">Вызов методов с параметрами</span><span class="sxs-lookup"><span data-stu-id="b169a-491">Calling Methods with Parameters</span></span>

<span data-ttu-id="b169a-492">По мере прочтения в этой статье объекты, с которыми Вы запрограммированы, могут иметь методы.</span><span class="sxs-lookup"><span data-stu-id="b169a-492">As you read earlier in this article, the objects that you program with can have methods.</span></span> <span data-ttu-id="b169a-493">Например, объект `Database` может иметь метод `Database.Connect`.</span><span class="sxs-lookup"><span data-stu-id="b169a-493">For example, a `Database` object might have a `Database.Connect` method.</span></span> <span data-ttu-id="b169a-494">Многие методы также имеют один или несколько параметров.</span><span class="sxs-lookup"><span data-stu-id="b169a-494">Many methods also have one or more parameters.</span></span> <span data-ttu-id="b169a-495">*Параметр* — это значение, передаваемое в метод, чтобы позволить методу завершить свою задачу.</span><span class="sxs-lookup"><span data-stu-id="b169a-495">A *parameter* is a value that you pass to a method to enable the method to complete its task.</span></span> <span data-ttu-id="b169a-496">Например, рассмотрим объявление метода `Request.MapPath`, который принимает три параметра:</span><span class="sxs-lookup"><span data-stu-id="b169a-496">For example, look at a declaration for the `Request.MapPath` method, which takes three parameters:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample64.cs)]

<span data-ttu-id="b169a-497">(Строка была заключена в оболочку, чтобы сделать ее более удобочитаемой.</span><span class="sxs-lookup"><span data-stu-id="b169a-497">(The line has been wrapped to make it more readable.</span></span> <span data-ttu-id="b169a-498">Помните, что разрывы строк можно поместить почти в любое место, за исключением строк, заключенных в кавычки.)</span><span class="sxs-lookup"><span data-stu-id="b169a-498">Remember that you can put line breaks almost any place except inside strings that are enclosed in quotation marks.)</span></span>

<span data-ttu-id="b169a-499">Этот метод возвращает физический путь на сервере, соответствующий указанному виртуальному пути.</span><span class="sxs-lookup"><span data-stu-id="b169a-499">This method returns the physical path on the server that corresponds to a specified virtual path.</span></span> <span data-ttu-id="b169a-500">Три параметра для метода: `virtualPath`, `baseVirtualDir`и `allowCrossAppMapping`.</span><span class="sxs-lookup"><span data-stu-id="b169a-500">The three parameters for the method are `virtualPath`, `baseVirtualDir`, and `allowCrossAppMapping`.</span></span> <span data-ttu-id="b169a-501">(Обратите внимание, что в объявлении указаны параметры с типами данных, которые они будут принимать.) При вызове этого метода необходимо указать значения для всех трех параметров.</span><span class="sxs-lookup"><span data-stu-id="b169a-501">(Notice that in the declaration, the parameters are listed with the data types of the data that they'll accept.) When you call this method, you must supply values for all three parameters.</span></span>

<span data-ttu-id="b169a-502">Синтаксис Razor предоставляет два варианта передачи параметров в метод: *позиционированные параметры* и *именованные параметры*.</span><span class="sxs-lookup"><span data-stu-id="b169a-502">The Razor syntax gives you two options for passing parameters to a method: *positional parameters* and *named parameters*.</span></span> <span data-ttu-id="b169a-503">Чтобы вызвать метод с помощью методов с параметрами, необходимо передать параметры в определенном порядке, указанном в объявлении метода.</span><span class="sxs-lookup"><span data-stu-id="b169a-503">To call a method using positional parameters, you pass the parameters in a strict order that's specified in the method declaration.</span></span> <span data-ttu-id="b169a-504">(Обычно этот порядок вы узнаете, читая документацию по методу.) Необходимо соблюдать порядок следования, и при необходимости нельзя пропустить какие-либо &#8212; параметры, передавая пустую строку (`""`) или `null` для параметра, для которого у вас нет значения.</span><span class="sxs-lookup"><span data-stu-id="b169a-504">(You would typically know this order by reading documentation for the method.) You must follow the order, and you can't skip any of the parameters &#8212; if necessary, you pass an empty string (`""`) or `null` for a positional parameter that you don't have a value for.</span></span>

<span data-ttu-id="b169a-505">В следующем примере предполагается, что на веб-сайте есть папка с именем *Scripts* .</span><span class="sxs-lookup"><span data-stu-id="b169a-505">The following example assumes you have a folder named *scripts* on your website.</span></span> <span data-ttu-id="b169a-506">Код вызывает метод `Request.MapPath` и передает значения для трех параметров в правильном порядке.</span><span class="sxs-lookup"><span data-stu-id="b169a-506">The code calls the `Request.MapPath` method and passes values for the three parameters in the correct order.</span></span> <span data-ttu-id="b169a-507">Затем отображается полученный сопоставленный путь.</span><span class="sxs-lookup"><span data-stu-id="b169a-507">It then displays the resulting mapped path.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample65.cshtml)]

<span data-ttu-id="b169a-508">Если у метода много параметров, можно сделать код более читаемым, используя именованные параметры.</span><span class="sxs-lookup"><span data-stu-id="b169a-508">When a method has many parameters, you can keep your code more readable by using named parameters.</span></span> <span data-ttu-id="b169a-509">Чтобы вызвать метод с использованием именованных параметров, укажите имя параметра, за которым следует двоеточие (:), а затем значение.</span><span class="sxs-lookup"><span data-stu-id="b169a-509">To call a method using named parameters, you specify the parameter name followed by a colon (:), and then the value.</span></span> <span data-ttu-id="b169a-510">Преимущество именованных параметров заключается в том, что их можно передавать в любом порядке.</span><span class="sxs-lookup"><span data-stu-id="b169a-510">The advantage of named parameters is that you can pass them in any order you want.</span></span> <span data-ttu-id="b169a-511">(Недостаток заключается в том, что вызов метода не является компактным.)</span><span class="sxs-lookup"><span data-stu-id="b169a-511">(A disadvantage is that the method call is not as compact.)</span></span>

<span data-ttu-id="b169a-512">В следующем примере вызывается тот же метод, что и выше, но для предоставления значений используются именованные параметры:</span><span class="sxs-lookup"><span data-stu-id="b169a-512">The following example calls the same method as above, but uses named parameters to supply the values:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample66.cshtml)]

<span data-ttu-id="b169a-513">Как видите, параметры передаются в другом порядке.</span><span class="sxs-lookup"><span data-stu-id="b169a-513">As you can see, the parameters are passed in a different order.</span></span> <span data-ttu-id="b169a-514">Однако если запустить предыдущий пример и этот пример, они будут возвращать одно и то же значение.</span><span class="sxs-lookup"><span data-stu-id="b169a-514">However, if you run the previous example and this example, they'll return the same value.</span></span>

<a id="ID_HandlingErrors"></a>
## <a name="handling-errors"></a><span data-ttu-id="b169a-515">Обработка ошибок</span><span class="sxs-lookup"><span data-stu-id="b169a-515">Handling Errors</span></span>

### <a name="try-catch-statements"></a><span data-ttu-id="b169a-516">Операторы Try-Catch</span><span class="sxs-lookup"><span data-stu-id="b169a-516">Try-Catch Statements</span></span>

<span data-ttu-id="b169a-517">Часто в коде есть операторы, которые могут завершиться ошибкой по причинам, находящимся за пределами элемента управления.</span><span class="sxs-lookup"><span data-stu-id="b169a-517">You'll often have statements in your code that might fail for reasons outside your control.</span></span> <span data-ttu-id="b169a-518">Пример:</span><span class="sxs-lookup"><span data-stu-id="b169a-518">For example:</span></span>

- <span data-ttu-id="b169a-519">Если код пытается создать файл или получить к нему доступ, могут возникать все виды ошибок.</span><span class="sxs-lookup"><span data-stu-id="b169a-519">If your code tries to create or access a file, all sorts of errors might occur.</span></span> <span data-ttu-id="b169a-520">Возможно, нужный файл не существует, он может быть заблокирован, код может не иметь разрешений и т. д.</span><span class="sxs-lookup"><span data-stu-id="b169a-520">The file you want might not exist, it might be locked, the code might not have permissions, and so on.</span></span>
- <span data-ttu-id="b169a-521">Аналогично, если код пытается обновить записи в базе данных, могут возникнуть проблемы с разрешениями, подключение к базе данных может быть удалено, данные для сохранения могут быть недействительными и т. д.</span><span class="sxs-lookup"><span data-stu-id="b169a-521">Similarly, if your code tries to update records in a database, there can be permissions issues, the connection to the database might be dropped, the data to save might be invalid, and so on.</span></span>

<span data-ttu-id="b169a-522">В терминах программирования эти ситуации называются *исключениями*.</span><span class="sxs-lookup"><span data-stu-id="b169a-522">In programming terms, these situations are called *exceptions*.</span></span> <span data-ttu-id="b169a-523">Если код сталкивается с исключением, он создает (выдает) сообщение об ошибке, которое лучше подойдет для пользователей:</span><span class="sxs-lookup"><span data-stu-id="b169a-523">If your code encounters an exception, it generates (throws) an error message that's, at best, annoying to users:</span></span>

![Razor — Img14](introducing-razor-syntax-c/_static/image14.jpg)

<span data-ttu-id="b169a-525">В ситуациях, когда код может столкнуться с исключениями и во избежание сообщений об ошибках этого типа, можно использовать инструкции `try/catch`.</span><span class="sxs-lookup"><span data-stu-id="b169a-525">In situations where your code might encounter exceptions, and in order to avoid error messages of this type, you can use `try/catch` statements.</span></span> <span data-ttu-id="b169a-526">В инструкции `try` выполняется проверка кода.</span><span class="sxs-lookup"><span data-stu-id="b169a-526">In the `try` statement, you run the code that you're checking.</span></span> <span data-ttu-id="b169a-527">В одной или нескольких инструкциях `catch` можно найти определенные ошибки (определенные типы исключений), которые могли произойти.</span><span class="sxs-lookup"><span data-stu-id="b169a-527">In one or more `catch` statements, you can look for specific errors (specific types of exceptions) that might have occurred.</span></span> <span data-ttu-id="b169a-528">Можно включить столько `catch` инструкций, сколько необходимо для поиска ожидаемых ошибок.</span><span class="sxs-lookup"><span data-stu-id="b169a-528">You can include as many `catch` statements as you need to look for errors that you are anticipating.</span></span>

> [!NOTE]
> <span data-ttu-id="b169a-529">Рекомендуется избегать использования метода `Response.Redirect` в инструкциях `try/catch`, так как это может вызвать исключение на странице.</span><span class="sxs-lookup"><span data-stu-id="b169a-529">We recommend that you avoid using the `Response.Redirect` method in `try/catch` statements, because it can cause an exception in your page.</span></span>

<span data-ttu-id="b169a-530">В следующем примере показана страница, которая создает текстовый файл при первом запросе, а затем отображает кнопку, которая позволяет пользователю открыть файл.</span><span class="sxs-lookup"><span data-stu-id="b169a-530">The following example shows a page that creates a text file on the first request and then displays a button that lets the user open the file.</span></span> <span data-ttu-id="b169a-531">В этом примере преднамеренно используется неправильное имя файла, что вызовет исключение.</span><span class="sxs-lookup"><span data-stu-id="b169a-531">The example deliberately uses a bad file name so that it will cause an exception.</span></span> <span data-ttu-id="b169a-532">Код включает `catch` операторы для двух возможных исключений: `FileNotFoundException`, которое происходит, если имя файла является недопустимым, и `DirectoryNotFoundException`, что происходит, если ASP.NET даже не может найти папку.</span><span class="sxs-lookup"><span data-stu-id="b169a-532">The code includes `catch` statements for two possible exceptions: `FileNotFoundException`, which occurs if the file name is bad, and `DirectoryNotFoundException`, which occurs if ASP.NET can't even find the folder.</span></span> <span data-ttu-id="b169a-533">(Чтобы увидеть, как она работает, когда все работает правильно, можно убрать комментарий к оператору в этом примере.)</span><span class="sxs-lookup"><span data-stu-id="b169a-533">(You can uncomment a statement in the example in order to see how it runs when everything works properly.)</span></span>

<span data-ttu-id="b169a-534">Если код не обработал исключение, появится страница ошибки, как на предыдущем снимке экрана.</span><span class="sxs-lookup"><span data-stu-id="b169a-534">If your code didn't handle the exception, you would see an error page like the previous screen shot.</span></span> <span data-ttu-id="b169a-535">Однако раздел `try/catch` помогает предотвратить возникновение ошибок такого типа.</span><span class="sxs-lookup"><span data-stu-id="b169a-535">However, the `try/catch` section helps prevent the user from seeing these types of errors.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample67.cshtml)]

## <a name="additional-resources"></a><span data-ttu-id="b169a-536">Дополнительные ресурсы</span><span class="sxs-lookup"><span data-stu-id="b169a-536">Additional Resources</span></span>

<span data-ttu-id="b169a-537">**Программирование с помощью Visual Basic**</span><span class="sxs-lookup"><span data-stu-id="b169a-537">**Programming with Visual Basic**</span></span>

[<span data-ttu-id="b169a-538">Приложение. Visual Basic язык и синтаксис</span><span class="sxs-lookup"><span data-stu-id="b169a-538">Appendix: Visual Basic Language and Syntax</span></span>](https://go.microsoft.com/fwlink/?LinkId=202908)

<span data-ttu-id="b169a-539">**Справочная документация**</span><span class="sxs-lookup"><span data-stu-id="b169a-539">**Reference Documentation**</span></span>

[<span data-ttu-id="b169a-540">ASP.NET</span><span class="sxs-lookup"><span data-stu-id="b169a-540">ASP.NET</span></span>](https://msdn.microsoft.com/library/ee532866.aspx)

[<span data-ttu-id="b169a-541">C#Языке</span><span class="sxs-lookup"><span data-stu-id="b169a-541">C# Language</span></span>](https://msdn.microsoft.com/library/kx37x362.aspx)
