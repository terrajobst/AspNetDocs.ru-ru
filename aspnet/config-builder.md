...
* `directoryPath` -Required. Указывает путь для поиска значения. Docker для Windows, секреты хранятся в *C:\ProgramData\Docker\secrets* каталог по умолчанию.
