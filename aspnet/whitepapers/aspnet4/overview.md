...
В ASP.NET 4, можно задать *controlRenderingCompatabilityVersion* значение «3.5», как показано в следующем примере:
...
В ASP.NET 4, можно задать *controlRenderingCompatabilityVersion* значение «4.0». Только элементы управления, работающие в этом случае *входной* элементы будут отображаться *отключена* атрибут элемента управления *включено* свойству *false* . Элементы управления, которые не отображают HTML *входной* вместо отображения элементов *класс* атрибут, который ссылается на класс CSS, который можно использовать для определения отключенный внешний вид элемента управления. Например *метка* элемента, показан в предыдущем примере создаст следующую разметку:
