...
Чтобы заставить веб-API считывать сложный тип из универсального кода ресурса (URI), добавьте атрибут **[фромури]** в параметр. В следующем примере определяется `GeoPoint` тип, а также метод контроллера, который `GeoPoint` получает из URI.
...
В следующем коде показан `GeoPoint` класс, представляющий географическую точку, а также **TypeConverter** , который преобразует строки в `GeoPoint` экземпляры. Класс снабжен атрибутом **[TypeConverter]** для указания преобразователя типов. `GeoPoint` (Этот пример был связан с записью блога Mike в блоге [о том, как выполнить привязку к пользовательским объектам в сигнатурах действий в MVC/WebAPI](https://blogs.msdn.com/b/jmstall/archive/2012/04/20/how-to-bind-to-custom-objects-in-action-signatures-in-mvc-webapi.aspx).)

[!code-csharp[Main](parameter-binding-in-aspnet-web-api/samples/sample6.cs)]

Теперь веб-API будет `GeoPoint` рассматриваться как простой тип, то есть будет пытаться `GeoPoint` привязать параметры из универсального кода ресурса (URI). В параметре не нужно включать **[фромури]** .
...
Также будет определено перечисление, указывающее, следует ли получить eTag из `if-match` заголовка `if-none-match` или заголовка.
...
Функция должна возвращать `null` значение для параметров, в которых привязка неприменима.
...
[Пример пользовательской привязки параметра](http://aspnet.codeplex.com/sourcecontrol/latest#Samples/WebApi/CustomParameterBinding/ReadMe.txt)
